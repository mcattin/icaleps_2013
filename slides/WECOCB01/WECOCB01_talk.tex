...
\begin{frame}{Outline}
  ...
  \note[item]{Then I'll quickly introduce Open Hardware and HOW we use it to develop the new kit.}
  ...
  \note[item]{And I will finish with some perspectives and conclusions.}
...
\begin{frame}{Use standards}
  ...
  \begin{block}{Based on standards}
    \begin{itemize}
    \item Platform bus: \textbf{VME, PCI, PCIe, PXIe}
    \item \textbf{FMC} (FPGA Mezzanine Card, ANSI/VITA 57.1)
    \item \textbf{Wishbone} FPGA internal bus
    \item \textbf{Linux} device drivers
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Contribute to standards}
    ...
    \item ZIO Linux framework for DAQ and CTL hardware: \\ \textbf{RFC} made to \textbf{Linux} Kernel list
    \end{itemize}
  \end{block}
...
\begin{frame}{New concepts}
  ...
  \note[item]{It's a set of structures describing the gateware architecture.}
